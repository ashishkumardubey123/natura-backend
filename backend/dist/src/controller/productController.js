...
async function getProductFilters(req, res) {
    try {
        ...
        const [therapyRows] = await db.query("SELECT DISTINCT therapy FROM products WHERE therapy IS NOT NULL AND therapy != ''");
        // 2. Database se unique Dosage Form (Product Form) nikalo, empty/null values ignore kardo
        const [dosageRows] = await db.query("SELECT DISTINCT dosageForm FROM products WHERE dosageForm IS NOT NULL AND dosageForm != ''");
        // Objects ke array ko simple string ke array me convert karo
        // e.g., [{ therapy: 'Cardiac' }, ...] -> ['Cardiac', ...]
        const therapyFilters = therapyRows.map((row) => row.therapy);
        const dosageFilters = dosageRows.map((row) => row.dosageForm);
