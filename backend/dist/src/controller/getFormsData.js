...
async function GetformsData(req, res) {
    try {
        ...
        const formatData = (rows, typeName, tableName, mapRow) => {
            return rows.map((row) => ({
                id: row.id,
                tableName,
                date: getRowDate(row),
        ...
        let allForms = [
            ...formatData(GeneralInquiry, "General Inquiry", "general_inquiries", (row) => ({
                name: row.full_name || "",
                email: row.email || "",
                phone: row.phone || "",
                message: row.message || ""
            })),
            ...formatData(ExportQuery, "Export Query", "export_queries", (row) => ({
                name: row.full_name || "",
                email: row.email || "",
                phone: row.phone || "",
            ...
            ...formatData(SupplierRegistration, "Supplier Registration", "supplier_registrations", (row) => ({
                company: row.company_name || "",
            ...
            ...formatData(BusinessPartnership, "Business Partnership", "business_partners", (row) => ({
                name: row.full_name || "",
                email: row.email || "",
                phone: row.phone || "",
                company: row.company_name || "",
...
async function UpdateFormStatus(req, res) {
    try {
        ...
        const [result] = await db.query(`UPDATE ?? SET status = ? WHERE id = ?`, [tableName, status, id]);
        return res.status(200).json({
            success: true,
