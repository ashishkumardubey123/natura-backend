...
async function getAdminEmails() {
    try {
        const [admins] = await db.query("SELECT Email FROM admins WHERE Role = 'SuperAdmin' OR status = 'live'");
...
async function GeneralInquiry(req, res) {
    ...
    try {
        await db.query("INSERT INTO general_inquiries (full_name, email, phone, message) VALUES (?, ?, ?, ?)", [Name, Email, Phone, Message]);
        const adminEmails = await getAdminEmails();
        const detailsHtml = tableRow("Full Name", Name) +
...
async function SupplierRegistration(req, res) {
    ...
    if (!Email?.trim())
        missingFields.push("Email");
    if (!SupplyCategory?.trim())
        missingFields.push("SupplyCategory");
    if (!CompanyProfile?.trim())
        missingFields.push("CompanyProfile");
    if (missingFields.length > 0) {
        return res.status(400).json({
            message: `Required fields missing: ${missingFields.join(", ")}`,
            missing: missingFields
    ...
    try {
        await db.query("INSERT INTO supplier_registrations (company_name, contact_person, email, phone, supply_category, company_profile) VALUES (?, ?, ?, ?, ?, ?)", [Company, Name, Email, Phone, SupplyCategory, CompanyProfile]);
        const adminEmails = await getAdminEmails();
        const detailsHtml = tableRow("Company Name", Company) +
...
async function ExportQuery(req, res) {
    ...
    try {
        await db.query(`INSERT INTO export_queries 
    (full_name, email, phone, company_name, target_country, products_of_interest, additional_details) 
    VALUES (?, ?, ?, ?, ?, ?, ?)`, [Name, Email, Phone, Company, Country, Products, Details]);
        const adminEmails = await getAdminEmails();
        const detailsHtml = tableRow("Client Name", Name) +
            tableRow("Email", Email) +
...
async function BusinessPartnership(req, res) {
    ...
    try {
        await db.query(`INSERT INTO business_partners 
  (full_name, email, phone, company_name, partnership_type, proposal_details)
  VALUES (?, ?, ?, ?, ?, ?)`, [Name, Email, Phone, Company, Partnership, Details]);
        const adminEmails = await getAdminEmails();
        const detailsHtml = tableRow("From", Name) +
            tableRow("Email", Email) +
            tableRow("Company", Company) +
